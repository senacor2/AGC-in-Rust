...
pub fn moon_position(t: Met) -> Vec3 {
    ...
    let l_prime_deg = norm_deg(
        218.316_447_7 + 481_267.881_234_21 * t_c - 0.001_578_6 * t2 + t3 / 538_841.0
            - t4 / 65_194_000.0,
    );

    // D — Moon's mean elongation.
    let d_deg = norm_deg(
        297.850_192_1 + 445_267.111_403_4 * t_c - 0.001_881_9 * t2 + t3 / 545_868.0
            - t4 / 113_065_000.0,
    );

    // M — Sun's mean anomaly.
    let m_deg =
        norm_deg(357.529_109_2 + 35_999.050_290_9 * t_c - 0.000_153_6 * t2 + t3 / 24_490_000.0);

    // M' — Moon's mean anomaly.
    let m_prime_deg = norm_deg(
        134.963_396_4 + 477_198.867_505_5 * t_c + 0.008_741_4 * t2 + t3 / 69_699.0
            - t4 / 14_712_000.0,
    );
    ...
    //     ascending node).
    let f_deg = norm_deg(
        93.272_095_0 + 483_202.017_523_3 * t_c - 0.003_653_9 * t2 - t3 / 3_526_000.0
            + t4 / 863_310_000.0,
    );
    ...
    let epsilon_deg =
        23.439_291_111 - 0.013_004_167 * t_c - 0.000_000_164 * t2 + 0.000_000_504 * t3;
...
mod tests {
    ...
    fn tc_moon_6_launch_position_approximate() {
        ...
        assert!(
            x < 0.0,
            "TC-MOON-6 SIGN FAIL: x = {x:.3e} m (expected x < 0)"
        );
        assert!(
            y > 0.0,
            "TC-MOON-6 SIGN FAIL: y = {y:.3e} m (expected y > 0)"
        );
        assert!(
            z > 0.0,
            "TC-MOON-6 SIGN FAIL: z = {z:.3e} m (expected z > 0)"
        );
