...
impl Executive {
    ...
    ///
    /// In the original AGC, NOVAC and FINDVAC were distinct entry points (the
    /// latter allocated a VAC scratchpad for the interpretive language). The
    /// interpreter is eliminated in this port (ADR-001), so only one entry
    /// point exists.
    pub fn create_job(
        &mut self,
